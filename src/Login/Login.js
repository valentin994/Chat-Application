    ...
    if (register) {
        formFields =
            <div>
                ...
                <input type="text" onChange={(e) => setUsername(e.target.value)} value={username} className="rounded-sm px-4 py-3 mt-3 placeholder-white focus:outline-none bg-gray-500 text-white w-full" placeholder="Name" />
                ...
                <div>
                    <button className="mt-6 px-4 py-1 text-white bg-green-400 hover:bg-green-500"
                        type="submit">Sign in</button> <Link onClick={() => setRegister(false)} className="text-white underline p-2 text-xs">You don't have an account? Register</Link>
                </div>
            </div>
    }
    return (
